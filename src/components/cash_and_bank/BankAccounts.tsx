...
export const BankAccounts: React.FC<BankAccountsProps> = ({ isPrivate }) => {
  ...
  const handleSaveBalance = (e: React.MouseEvent, id: string) => {
    ...
    setAccounts(prev => prev.map(acc =>
      acc.id === id
        ? { ...acc, balance: val, lastUpdated: 'just now' }
        : acc
    ));
  ...
  return (
    <div className="space-y-6 fade-in p-2">
              ...
              >
                ...
                >
                  <div className="overflow-hidden">
                    <div className="px-5 pb-5 pt-0 bg-white rounded-b-3xl">
                      <div className="border-t border-[#C3C6CE]/30 pt-5 grid grid-cols-2 gap-6">
                        <div className="space-y-1">
                          <span className="block text-[10px] font-bold text-[#73777E] uppercase" style={{ letterSpacing: '1px' }}>
                            Account Number
                          </span>
                          <span className="block text-sm font-bold text-orelio-navy" style={{ letterSpacing: '1px' }}>
                            {account.accountNumber?.trim() ? f(account.accountNumber) : '-'}
                          </span>
                        </div>
                        <div className="space-y-1">
                          <span className="block text-[10px] font-bold text-[#73777E] uppercase" style={{ letterSpacing: '1px' }}>
                          ...
                          <span className="block text-sm font-bold text-orelio-navy" style={{ letterSpacing: '1px' }}>
                            {account.ifscCode?.trim() ? f(account.ifscCode) : '-'}
                          </span>
                        </div>
                      </div>
                    </div>
      ...
      <div className="bg-[#00162A] text-white rounded-3xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        {/* Glow graphic overlay in background */}
            ...
            <div className="flex-1 flex flex-col space-y-6 py-4 overflow-y-auto min-h-0 pr-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 flex-shrink-0">
                <div className="md:col-span-2 space-y-2">
                  <label className="block text-[11px] font-bold text-[#3F4945] tracking-wider uppercase">Bank Name</label>
                  <input
                    type="text"
                    required
                    value={formBankName}
                ...
                <div className="space-y-2">
                  <label className="block text-[11px] font-bold text-[#3F4945] tracking-wider uppercase">Account Type</label>
                  <select
                    value={formAccountType}
                    onChange={(e) => setFormAccountType(e.target.value)}
                    className="w-full px-[11px] py-[7px] rounded-xl bg-orelio-light-gray/60 border-2 border-[#C3C6CE]/15 hover:border-[#C3C6CE]/35 text-sm text-orelio-navy font-semibold focus:outline-none focus:bg-white focus:border-[#006A65] transition-all cursor-pointer"
                  >
              ...
              <div className="space-y-2 flex-shrink-0">
                <label className="block text-[11px] font-bold text-[#3F4945] tracking-wider uppercase">Balance (INR)</label>
                <input
                  type="number"
                  required
                  value={formBalance}
